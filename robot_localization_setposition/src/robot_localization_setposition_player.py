...
left_start = [5.5, 0.87, math.pi/2.0]
right_start = [10.5, 0.87, math.pi/2.0]
...
if __name__ == "__main__":
    ...
    rospy.Subscriber('/alice/player/penalty',UInt8,penalty_callback)
    rospy.Subscriber('/alice/keeper/penalty',UInt8,other_penalty_callback)
    ...
    while not rospy.is_shutdown():
        ...
        if state == 0: #init
            ...
            if penalty != 0:  # if there is a player already got removal penalty
                if team_index == 0:
                    if goal_x_offset < 320 : 
                        state_determine_msg.x = left_offense_start[0]
                        state_determine_msg.y = left_offense_start[1]
